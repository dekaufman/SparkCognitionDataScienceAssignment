...
library(PRROC)
roc <- roc.curve(scores.class0 = as.data.frame(qqq6$votes)$no, weights.class0 = qqq6_imputed$responded %>% as.numeric(), curve = TRUE)
roc
plot(roc)
...
qqq10 <- data.frame(D = qqq6_imputed$responded %>% as.numeric(),
                    M1 = as.data.frame(qqq6$votes)$yes, stringsAsFactors = FALSE)

basicplot <- ggplot(qqq10, aes(d = D, m = M1)) + geom_roc() + 
